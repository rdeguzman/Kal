...
@protocol KalViewControllerExternalDelegate;
...
@interface KalViewController : UIViewController <KalViewDelegate, KalDataSourceCallbacks>
{
  ...
  id <KalViewControllerExternalDelegate> delegateExternal;
}
...
@property (nonatomic, assign) id <KalViewControllerExternalDelegate> delegateExternal;

- (id)initWithSelectedDate:(NSDate*)date;
- (id)initWithFrame:(CGRect)frame;
- (id)initWithFrame:(CGRect)frame initWithSelectedDate:(NSDate *)selectedDate; // designated initializer. When the calendar is first displayed to the user, the month that contains 'selectedDate' will be shown and the corresponding tile for 'selectedDate' will be automatically selected.
...
@protocol KalViewControllerExternalDelegate <NSObject>
@optional
